...
const tokenizeYamlLine = (line: string): Token[] => {
  ...
  while (remaining.length > 0) {
    ...
    if (remaining.startsWith('#')) {
      ...
      tokens.push({ text: remaining.substring(0, commentEnd), type: 'comment' });
...
const tokenizeYamlContent = (content: string, ctx: CanvasRenderingContext2D): TokenizedLine[] => {
...
// Text wrapping function that respects tokenization
const wrapTextWithTokens = (lines: TokenizedLine[], maxWidth: number, ctx: CanvasRenderingContext2D): TokenizedLine[] => {
  const wrappedLines: TokenizedLine[] = [];
  
  for (const line of lines) {
    if (line.tokens.length === 0) {
      wrappedLines.push({ tokens: [], width: 0 });
      continue;
    }
    ...
    for (const token of tokens) {
      ...
      if (currentX + metrics.width > maxWidth && currentLine !== '') {
        // Wrap current line
        if (currentLine !== '') {
          newTokens.push({ text: currentLine, type: 'default' });
        }
    ...
    const width = newTokens.reduce((sum, token) => {
      const metrics = ctx.measureText(token.text);
      return sum + metrics.width;
    }, 0);
    
    wrappedLines.push({ tokens: newTokens, width });
  }
...
const render = () => {
  ...
  try {
    ...
    if (!ctx) {
      self.postMessage({ success: false, error: 'Failed to get context' } as WorkerResponse);
      return;
    }
    ...
    const totalHeight = tokenizedLines.reduce((sum, line) => {
    ...
    self.postMessage({ success: false, error: error.message } as WorkerResponse);
  }
};
